...
def main(babble_level, capacity, batch_size, ship, order_point):
	...
	with open('./data/data_now.txt', 'r') as data:
		for demand in data:
			count_day += 1
			demand = int(demand)
			...
			if count_day == 220:
				c.order_point = 500 #means "After 220 days, change order point to 500"
				print('change order point to',c.order_point)
			if count_day == 250:
				c.order_point = 200
			if count_day == 290:
				c.order_point = 0

	print('Final gain,', c.cash / 1000000)
	#print(c.order_point,',',c.batch_size,',',c.cash / 1000000)
	print('lost history',c.lost_history)
...
if __name__ == '__main__':
	...
	for ORDER_POINT in range(100,3000,100):
		for BATCH_SIZE in range(50,1000,50):
			main(
				babble_level=args.b,
				capacity=CAPACITY,
				batch_size=BATCH_SIZE,
